...
def xception(pretrained=True, downsample_factor=16):
    ...
    if pretrained:
        # Xception pretrained weights are not available via torchvision.
        # Place weights at model_data/xception_pytorch_imagenet.pth,
        # or use download_weights() from utils/utils.py.
        model_path = './model_data/xception_pytorch_imagenet.pth'
        if os.path.exists(model_path):
            model.load_state_dict(torch.load(model_path, map_location='cpu'), strict=False)
        else:
            print("Xception pretrained weights not found at './model_data/xception_pytorch_imagenet.pth'. "
                  "Training from scratch. Set pretrained=False to suppress this message.")
    return model
